...
import { getSyncHistoryView } from "@/actions/sync.actions";
...
const maxTime = momentTz
  .tz(timeZone)
  .set({ hour: 22, minute: 59, second: 59, millisecond: 59 })
  .toDate();
...
const HistoryCalendar = forwardRef(
  ({ date, setDate, cloudFunction }: Props, ref) => {
    ...
    const refreshEvents = useCallback(async () => {
      ...
      const history = await getSyncHistoryView(cloudFunction, view, dateStr);
      const historyEventsPromise = history.map((his) => {
        ...
        const startDate = setMinutesDate(his.last_cf_execution, minutes);
        return {
    ...
    return (
      <div id="sincronizaciones" className="w-full">
        <Calendar
          culture="es"
          date={date}
          defaultDate={date}
...
HistoryCalendar.displayName = "HistoryCalendarSync";
