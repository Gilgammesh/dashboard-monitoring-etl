...
export const getMassiveHistoryView = async (
  ...
  try {
    const results = await prisma.$queryRaw<etl_cloudfunc_log_sync[]>`
      ...
      WHERE
        ...
        AND cf_scheduler = ${CloudFunctionScheduler.Massive}::cf_scheduler
...
export const getMassiveHistoryDetail = async (cf_execution_id: string) => {
  try {
    const results = await prisma.$queryRaw<etl_cloudfunc_log_sync[]>`
      SELECT
