...
export const getRecoveryHistoryView = async (
  ...
  try {
    const results = await prisma.$queryRaw<etl_cloudfunc_log_sync[]>`
      ...
      WHERE
        ...
        AND cf_scheduler = ${CloudFunctionScheduler.Recovery}::cf_scheduler
    ...
    console.error("getRecoveryHistoryView Error", error);
    return [];
  }
};

export const getRecoveryHistoryDetail = async (cf_execution_id: string) => {
  try {
    const results = await prisma.$queryRaw<etl_cloudfunc_log_sync[]>`
      SELECT
    ...
    console.error("getRecoveryHistoryDetail Error", error);
    return [];
  }
};
